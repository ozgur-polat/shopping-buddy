...
def get_clusters(data, k=2, min_size=1, max_size=-1, num_iter=1):
    """
    gets data and other parameters
    generates clusters and assign members to clusters
    returns cluster memberships
    """
    if max_size ==-1:max_size==len(data)/2
    ...
    if best:
        ...
        return pd.Series(best_clusters)
    else:
